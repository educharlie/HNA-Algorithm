...
void duDebugDrawHierarchicalDivision(duDebugDraw* dd, const dtNavMesh& mesh, std::map<dtPolyRef, int> clusteredNodes)
{
	if (!dd) return;
	...
	for (int m = 0; m < mesh.getMaxTiles(); ++m)
	{
		const dtMeshTile* tile = mesh.getTile(m);
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			dtPolyRef ref = mesh.getPolyRefBase(tile)+i;

			if(clusteredNodes.size() == 0)
			{
				unsigned int color = duIntToCol(ref, 255);
				...
				for (int j = 0; j < pd->triCount; ++j)
				{
					...
					{
						if (t[k] < p->vertCount)
							dd->vertex(&tile->verts[p->verts[t[k]]*3], color);
						else
							dd->vertex(&tile->detailVerts[(pd->vertBase+t[k]-p->vertCount)*3], color);
					}
				}
			}
			else
			{
				...
				auto it = clusteredNodes.find(int(ref));
			
				if(it->second < 0 )
					continue;

				if(it != clusteredNodes.end())
				{
					unsigned int color = duIntToCol(it->second, 255);
