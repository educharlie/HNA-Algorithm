...
void duLogBuildTimes(rcContext& ctx, const int totalTimeUsec)
{
	...
	logLine(ctx, RC_TIMER_BUILD_HIERARCHICAL_GRAPH,	"- Build Hierarchical Graph", pc);
	logLine(ctx, RC_TIMER_GET_PARENT_NODES,			"- Get parent nodes", pc);
	logLine(ctx, RC_TIMER_LINK_START_END_NODES,		"- Link S and E nodes", pc);
	logLine(ctx, RC_TIMER_FIND_HIERACHICAL_PATH, 	"- Find hierarchical path", pc);
	logLine(ctx, RC_TIMER_GET_SUBPATH, 				"- Get subpath", pc);
	logLine(ctx, RC_TIMER_DELETE_START_END_NODES, 	"- Delete S and E nodes", pc);
