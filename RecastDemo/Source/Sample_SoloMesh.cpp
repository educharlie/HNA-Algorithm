...
void Sample_SoloMesh::handleHierarchySettings()
{
	Sample::handleHierarchySettings();

	imguiSeparator();

	char msg1[64];
	snprintf(msg1, 64, "Build Time: %.1fms", m_totalBuildTimeMs);
	imguiLabel(msg1);
...
void Sample_SoloMesh::handleDebugMode()
{
	...
	if (m_geom)
	{
		...
		valid[DRAWMODE_HIERARCHICAL] = m_dmesh != 0;
	}
	...
	if (imguiCheck("Hierarchical Division", m_drawMode == DRAWMODE_HIERARCHICAL, valid[DRAWMODE_HIERARCHICAL]))
		m_drawMode = DRAWMODE_HIERARCHICAL;
...
void Sample_SoloMesh::handleRender()
{
	...
	if (m_drawMode != DRAWMODE_NAVMESH_TRANS && m_drawMode != DRAWMODE_HIERARCHICAL)
	{
		// Draw mesh
	...
	if (m_dmesh && m_drawMode == DRAWMODE_HIERARCHICAL)
	{
		glDepthMask(GL_FALSE);
		duDebugDrawHierarchicalDivision(&dd, *m_navMesh, nodeCluster);
		glDepthMask(GL_TRUE);
	}
...
bool Sample_SoloMesh::handleHierarchy()
{
	// Reset build times gathering.
	m_ctx->resetTimers();
	...
	for (int i = 0; i < m_navMesh->getMaxTiles(); ++i)
	{
		...
		if (!tile->header) 
			continue;
		nodeCluster.clear();
		m_hnav.createHierarchicalGraph(int(m_max_levels),int(m_level), int(m_merged_polys), m_ctx, tile, m_navMesh, m_navQuery, nodeCluster);
	}
	...
	// Show performance stats.
	duLogBuildTimes(*m_ctx, m_ctx->getAccumulatedTime(RC_TIMER_TOTAL));
	m_ctx->log(RC_LOG_PROGRESS, ">> Ready");
	m_totalBuildTimeMs = m_ctx->getAccumulatedTime(RC_TIMER_TOTAL)/1000.0f;
